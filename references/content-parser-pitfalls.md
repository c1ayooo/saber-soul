...
## 13. 文档 URL 生成：obj_token ≠ node_token

飞书文档创建后返回两个 token：

```python
resp = POST /wiki/v2/spaces/{space_id}/nodes
# resp["node"]["node_token"]     # wiki 节点 ID（/wiki/... 路径用）
# resp["node"]["obj_token"]      # docx 文档 ID（/docx/... 路径用）
```

**错误：** `f"https://bytedance.feishu.cn/docx/{node_token}"` → 页面不存在（404）
**正确：** `f"https://bytedance.feishu.cn/docx/{obj_token}"` → 正常访问

**已在 feishu_doc.py 修复：** `doc_token = resp["node"].get("obj_token", node_token)` 优先用 obj_token，兼容没有 obj_token 的旧版本。

## 14. Mermaid 渲染替代 mmdc（无需 Node.js）

原实现依赖 `mmdc`（@mermaid-js/mermaid-cli），需要 Node.js + npm。2026 年改用 **mermaid.ink API** 替代：
...
**优势：** 零依赖、无需安装、支持所有 Mermaid 语法
**注意：** 必须设置 User-Agent header（否则 403）；通过 base64 编码而非 URL 参数

## 15. 图片插入飞书文档需要 drive:drive 权限

图片上传到飞书文档必须用 `drive/v1/medias/upload_all`（返回 `file_token`），而非 `im/v1/images`（返回 `image_key` 仅供消息使用）。

```python
# ✅ 文档图片（block_type=27）
POST /drive/v1/medias/upload_all
  files={"file": (name, f, "image/png")}
  data={"parent_type": "docx_image", "parent_node": "", "size": str(size)}
# → file_token（可用于 docx image block）
...
**权限要求：** 飞书应用需要开通 **`drive:drive`** 云文档权限并重新发布。当前 saber-soul 的 Feishu 应用缺少此权限，图片插入会返回 `1061004 forbidden`。

**已修复位置：** `FeishuDoc._upload_image()` 已切换为 drive API，`FeishuDoc._insert_image_block()` 已适配 POST children endpoint。

## 16. 示例 JSON 中的 CVE- 子串导致 API 文档误判为 CVE
