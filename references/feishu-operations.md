...
飞书 Wiki 空间不支持在 UI 中搜索和添加应用（机器人）为成员，必须通过 API 操作。

**两种 `member_type` 方式：**

#### 方式一：通过 app_id（推荐，无需获取 open_id）

直接用应用的 `cli_xxx` ID 添加：

```http
POST /open-apis/wiki/v2/spaces/{space_id}/members HTTP/1.1
Authorization: Bearer {user_access_token}
Content-Type: application/json

{
    "member_type": "appid",
    "member_id": "cli_aabb8be5ee781be3",
    "member_role": "admin"
}
```

`member_role` 可选值：`admin`（管理员）、`member`（普通成员）、`viewer`（查看者）、`commenter`（评论者）。

#### 方式二：通过 open_id（传统方式，需先获取 bot 的 open_id）
...
| 条件 | 说明 |
|------|------|
| Token 类型 | **必须使用 `user_access_token`**，`tenant_access_token` 没有 wiki 空间成员管理权限 |
| 用户权限 | 该用户必须是该知识库的已有管理员（admin） |
| 有效 `member_type` | `email`、`openid`、`unionid`、`openchat`、`opendepartmentid`、`userid`、`groupid`、`wikispaceid`、`appid` |

### 6.3 验证成员列表

```http
GET /open-apis/wiki/v2/spaces/{space_id}/members?page_size=50 HTTP/1.1
Authorization: Bearer {user_access_token}

# 响应（code=0）
# {"members": [{"member_id":"ou_xxx","member_role":"admin","member_type":"openid"}, ...]}
```
...
| 错误码 | 含义 | 处理 |
|--------|------|------|
| `99991668` | user_access_token 过期 | 重新走 OAuth 获取 token |
| `131005` | 节点不存在或权限不足 | 检查应用是否在空间成员列表中 |
| `131006` | 应用未加入 wiki 空间 | 先通过此流程添加应用 |

### 6.5 添加本应用（Hermes）为 wiki 空间成员

如果 Hermes 的 `tenant_access_token` 在访问 wiki 节点时报 `131005` 或 `1770032`，说明 Hermes 应用本身没有被加入 wiki 空间。处理方法：

1. 获取 Hermes 应用的 `app_id`（在 `~/.hermes/.env` 中为 `FEISHU_APP_ID`）
2. 用你的 `user_access_token` 调用 `POST /wiki/v2/spaces/{space_id}/members`，`member_type=appid`，`member_id=cli_xxx`
3. 添加完成后，Hermes 的 `tenant_access_token` 即可操作所有 wiki 节点
...
## 十、自定义配置（feishu_config.json 可选项）
